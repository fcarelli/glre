...
rule bwa_pe_ce11: # rm -f ... gets rid of temporary files left from a previously crashed run...
    ...
    shell: '''
        rm -f {output}_*
	bwa aln -t {threads} {params.genome_fa} {input[0]} > {output}_aln_0
	bwa aln -t {threads} {params.genome_fa} {input[1]} > {output}_aln_1
	bwa sampe -o 50000 {params.genome_fa} {output}_aln_0 {output}_aln_1  {input[0]} {input[1]} > {output}_sampe
	samtools view -b -@ {threads} {output}_sampe | samtools sort -@ {threads} -O sam -T {output}_tmpA -n - | samtools fixmate -O sam - - | samtools sort -@ {threads} -O bam -T {output}_tmpB - > {output}
        '''
