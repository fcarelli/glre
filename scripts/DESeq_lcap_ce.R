...
sample_ID_list=data.frame("sample" = c("annot_ce_lcap_wt_ya_rep1", "annot_ce_lcap_wt_ya_rep2", "annot_ce_lcap_glp1_ya_rep1", "annot_ce_lcap_glp1_ya_rep2"), "strain" = c("wt", "wt", "glp1", "glp1"))

strains=levels(sample_ID_list$strain)

bam_files=c()
for (i in c(1:dim(sample_ID_list)[1])){
  bam_files = c(bam_files, paste("relmapping/lcap/trim20.bwa_pe_ce11.rm_unmapped_pe.rm_contigs_ce11.rm_q10/", sample_ID_list[i,1], ".trim20.bwa_pe_ce11.rm_unmapped_pe.rm_contigs_ce11.rm_q10.bam", sep=""))
}
...
write.table(normalized_counts, file="gene_annotation/elegans.normalized_counts_DESeq.txt", sep="\t", quote=F, col.names=NA)
...
write.table(downreg, file = paste("gene_annotation/elegans.genes_downregulated_", mutant, "_vs_wt.txt", sep=""), quote=F, col.names = F, row.names = T, sep="\t")
write.table(upreg, file = paste("gene_annotation/elegans.genes_upregulated_", mutant, "_vs_wt.txt", sep=""), quote=F, col.names = F, row.names = T, sep="\t")
write.table(res, file = paste("gene_annotation/elegans.genes_DESeq_", mutant, "_vs_wt.txt", sep=""), quote=F, row.names = T, sep="\t")
