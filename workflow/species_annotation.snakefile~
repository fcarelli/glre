...
rule genome_download_additional_species:
  output:
    ...
    japonica='species/japonica/genome/japonica.fa',
  ...
  shell:
    ...
    wget http://download.caenorhabditis.org/v1/sequence/Caenorhabditis_japonica_C_japonica-7.0.1.scaffolds.fa.gz -O {output.japonica}.gz; gunzip {output.japonica}.gz
...
rule gene_annotation_download_all:
  output:
    ...
    japonica='species/japonica/gene_annotation/japonica.annotations.all.gff3.gz'
  shell:
    '''
    ...
    wget ftp://ftp.wormbase.org/pub/wormbase/species/c_japonica/gff/c_japonica.PRJNA12591.WS275.annotations.gff3.gz -O {output.japonica}
    '''
...
rule elegans_operons:
  input:
    gff = "species/elegans/gene_annotation/elegans.annotations.all.gff3.gz",
    longest_isoform = "species/elegans/gene_annotation/elegans.gene_annotation.coding_gene_longest_isoform.bed"
  output:
    all_gff = temp("species/elegans/gene_annotation/elegans.annotations.all.gff3"),
    operons = "species/gene_annotation/elegans.operons.bed",
    internal_genes = "species/elegans/gene_annotation/elegans.operons.internal.genes"
  shell:
    """
    gunzip -k {input.gff}
    awk 'BEGIN{{FS="\t|=|;"; OFS="\t";}}{{if ($2 == "operon") print $1, $4, $5, $10, $12, $7}}' {output.all_gff} > {output.operons}
    python scripts/operon_middle_genes.py {output.operons} {input.longest_isoform} {output.internal_genes}
    sort {output.internal_genes} > {output.internal_genes}.sorted
    mv {output.internal_genes}.sorted {output.internal_genes}
    """
...
rule protein_annotation_download_extended_species:
  output:
    ...
    japonica='species/japonica/protein_sequences/japonica.protein.longest_isoform.fa',
  ...
  shell:
    ...
    wget http://download.caenorhabditis.org/v1/sequence/Caenorhabditis_japonica_C_japonica-7.0.1.proteins.fa.gz -O {output.japonica}.to_clean.gz; gunzip {output.japonica}.to_clean.gz; awk 'BEGIN{{FS=" ";}}{{if ($1 ~ ">") print $1; else print $0}}' {output.japonica}.to_clean | fold -s -w60 > {output.japonica}.clean; rm {output.japonica}.to_clean; python scripts/other_species_protein_fix.py {output.japonica}.clean {output.japonica}; rm {output.japonica}.clean
...
rule repeatmodeler:
  ...
  shell:
    '''
    mkdir species/{wildcards.sample}/repeatmodeler/blastdb
    BuildDatabase -name species/{wildcards.sample}/repeatmodeler/blastdb/{wildcards.sample}_repeatmodeler {input[0]}
    cd species/{wildcards.sample}/repeatmodeler
    RepeatModeler -pa {resources.cpus} -database blastdb/{wildcards.sample}_repeatmodeler
    cp RM*/consensi.fa .
    '''
...
rule mitehunter:
  input:
    'species/{sample}/genome/{sample}.fa',
  output:
    temp('species/{sample}/mitehunter/{sample}.copy.fa'),
    temp('species/{sample}/mitehunter/{sample}.copy.fa.index'),
    temp('species/{sample}/mitehunter/{sample}.copy.fa.nhr'),
    temp('species/{sample}/mitehunter/{sample}.copy.fa.nin'),
    temp('species/{sample}/mitehunter/{sample}.copy.fa.nsq'),
  ...
  shell:
    '''
    cp {input[0]} {output[0]}
    cd species/{wildcards.sample}/mitehunter
    #MITE_Hunter_manager.pl -i {wildcards.sample}.copy.fa -g {wildcards.sample}_mitehunter -n 5 -w 1000 -L 80 -c {resources.cpus} -t 20 -l 2 -S 12345678
    #MITE_Hunter_manager.pl -i {wildcards.sample}.copy.fa -g {wildcards.sample}_mitehunter -n 5 -w 1000 -L 80 -c {resources.cpus} -t 50 -l 10 -S 12345678 -m 10
    MITE_Hunter_manager.pl -i {wildcards.sample}.copy.fa -g {wildcards.sample}_mitehunter -n 5 -w 1000 -L 80 -c {resources.cpus} -t 10 -l 1 -S 12345678
    cat {wildcards.sample}_mitehunter_Step8_* > {wildcards.sample}_mitehunter.mite.all.fa
    '''


rule essential_genes:
  input:
    'data/external_data/simplemine_results.txt',
  output:
    'data/external_data/essential_genes.tested.txt',
    'data/external_data/essential_genes.txt',
  shell:
    '''
    python scripts/essential_genes_parser.py {input} {output[0]} {output[1]}
    '''
